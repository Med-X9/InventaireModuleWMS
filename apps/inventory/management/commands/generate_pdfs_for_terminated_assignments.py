"""
Commande Django pour générer les PDFs de tous les assignments terminés.

Cette commande :
- Récupère tous les assignments avec le statut TERMINE
...
Exemple d'utilisation:
    python manage.py generate_pdfs_for_terminated_assignments --output-dir ./pdfs
    python manage.py generate_pdfs_for_terminated_assignments --output-dir ./pdfs --job-id 123
    python manage.py generate_pdfs_for_terminated_assignments --output-dir ./pdfs --inventory-id 456
    python manage.py generate_pdfs_for_terminated_assignments --output-dir ./pdfs --job-refs JOB-0001 JOB-0002
    python manage.py generate_pdfs_for_terminated_assignments --output-dir ./pdfs --file jobs_list.txt
    python manage.py generate_pdfs_for_terminated_assignments --output-dir ./pdfs --use-static
"""
...
# Liste statique des références de jobs pour lesquels générer les PDFs
...
class Command(BaseCommand):
    help = 'Génère les PDFs de tous les assignments terminés'
    ...
    def handle(self, *args, **options):
        ...
        if not job_refs:
            ...
            # Si on utilise la liste statique par défaut, ignorer les autres filtres
            job_id = None
            inventory_id = None
        ...
        # Récupérer les assignments terminés
        # Si job_refs est fourni, utiliser uniquement ces références (ignorer job_id et inventory_id)
        assignments = self._get_terminated_assignments(
        ...
        if not assignments.exists():
            self.stdout.write(self.style.WARNING('⚠️  Aucun assignment terminé trouvé'))
            return
        
        self.stdout.write(self.style.SUCCESS(f'📋 {assignments.count()} assignment(s) terminé(s) trouvé(s)'))
        if dry_run:
                ...
                if result['success']:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            ...
                            f'(Job: {assignment.job.reference}, Assignment: {assignment.reference})'
                        )
                    )
                else:
    ...
    def _get_terminated_assignments(
        self, 
        ...
        """
        Récupère les assignments terminés selon les filtres fournis.
        ...
        Returns:
            QuerySet des assignments terminés
        """
        queryset = Assigment.objects.filter(status='TERMINE').select_related(
            'job',
            'job__inventory',
            'counting',
            'personne',
