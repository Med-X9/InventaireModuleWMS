"""
Commande Django pour mettre les jobs et assignments au statut TRANSFERT
avec la date de transfert à la date/heure actuelle.

Exemple d'utilisation:
    python manage.py set_jobs_assignments_transfert --inventory-id 2 --warehouse-id 1
    python manage.py set_jobs_assignments_transfert --inventory-id 2 --warehouse-id 1 --dry-run
"""
...
class Command(BaseCommand):
    help = 'Met les jobs et assignments au statut TRANSFERT avec la date de transfert actuelle'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            nargs='+',
            help='Filtrer les jobs par statut avant modification (ex: --job-status-filter EN_ATTENTE PRET)',
        )
        parser.add_argument(
            '--assignment-status-filter',
            type=str,
            nargs='+',
            help='Filtrer les assignments par statut avant modification (ex: --assignment-status-filter EN_ATTENTE PRET)',
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('📋 Mise à jour des statuts vers TRANSFERT'))
        ...
        if dry_run:
            ...
            with transaction.atomic():
                for job in jobs:
                    if job.status == 'TRANSFERT':
                        jobs_already_transfert += 1
                        continue
                    
                    old_status = job.status
                    job.status = 'TRANSFERT'
                    # Mettre à jour la date de transfert
                    job.transfert_date = current_time
                    job.save()
                    jobs_updated += 1
                    
                    self.stdout.write(
                        f'  ✓ Job {job.reference}: {old_status} → TRANSFERT (transfert_date: {current_time})'
                    )
            
            # Mettre à jour les assignments
            assignments_updated = 0
            assignments_already_transfert = 0
            
            self.stdout.write(self.style.SUCCESS('\n🏭 Mise à jour des assignments...'))
            
            with transaction.atomic():
                for assignment in assignments:
                    if assignment.status == 'TRANSFERT':
                        assignments_already_transfert += 1
                        continue
                    
                    old_status = assignment.status
                    assignment.status = 'TRANSFERT'
                    assignment.transfert_date = current_time
                    # Mettre à jour aussi les autres dates si nécessaire
                    if not assignment.affecte_date:
                        assignment.affecte_date = current_time
                    if not assignment.date_start:
                    ...
                    self.stdout.write(
                        ...
                        f'{old_status} → TRANSFERT (transfert_date: {current_time})'
                    )
            ...
            self.stdout.write(f'  • Jobs déjà TRANSFERT: {jobs_already_transfert}')
            self.stdout.write(f'  • Assignments modifiés: {assignments_updated}')
            self.stdout.write(f'  • Assignments déjà TRANSFERT: {assignments_already_transfert}')
            self.stdout.write(f'  • Total jobs: {total_jobs}')
            self.stdout.write(f'  • Total assignments: {total_assignments}')
            self.stdout.write(f'  • Date de transfert: {current_time}')
        ...
        if dry_run:
            ...
            self.stdout.write(f'  • Date de transfert qui serait utilisée: {current_time}')
