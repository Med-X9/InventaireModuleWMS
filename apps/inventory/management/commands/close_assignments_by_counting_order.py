"""
Commande Django pour clôturer les assignments des comptages 1 et 2.

Cette commande applique la même logique que CloseJobView :
- Récupère tous les assignments avec counting order 1 ou 2 qui ne sont pas TERMINE
- Vérifie que tous les JobDetail liés au job et au counting sont TERMINE
- Clôture les assignments qui remplissent cette condition
- Synchronise les CountingDetail entre les deux countings si nécessaire
...
Exemple d'utilisation:
    python manage.py close_assignments_by_counting_order --inventory-id 2 --warehouse-id 1
    python manage.py close_assignments_by_counting_order --inventory-id 2 --warehouse-id 1 --dry-run
"""
...
class Command(BaseCommand):
    help = 'Clôture les assignments des comptages 1 et 2 si tous les JobDetail liés sont TERMINE'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('📋 Clôture des assignments des comptages 1 et 2'))
        ...
        # IMPORTANT: Trier par counting.order pour traiter d'abord counting 1, puis counting 2
        # Cela garantit que la synchronisation se fait après que les deux assignments soient TERMINE
        assignments = Assigment.objects.filter(
            ...
            'job__warehouse',
            'counting'
        ).order_by('counting__order', 'job_id')  # Trier par ordre de comptage (1 puis 2), puis par job
        ...
        for assignment in assignments:
            try:
                ...
                stats['skipped_non_terminated'] += result['skipped_non_terminated']
        ...
        self.stdout.write(f"  Assignments ignorés (JobDetail non TERMINE): {stats['skipped_non_terminated']}")
        self.stdout.write('')
    ...
    def process_assignment(self, assignment: Assigment, dry_run: bool = False) -> dict:
        """
        Traite un assignment en appliquant la même logique que CloseJobView :
        1. Vérifie que tous les JobDetail du job liés au même counting sont TERMINE
        2. Si oui, clôture l'assignment
        3. Synchronise les CountingDetail entre les deux countings si nécessaire
        ...
        # 1. Vérifier que tous les JobDetail du job liés au même counting sont TERMINE
        # IMPORTANT: Gérer les doublons (même job, même location, même counting)
        # Pour chaque combinaison unique (location, counting), vérifier qu'au moins un JobDetail est TERMINE
        job_details = JobDetail.objects.filter(
            job=job,
            counting=counting
        ).select_related('location')
        
        total_job_details = job_details.count()
        ...
        # Vérifier pour chaque location unique qu'au moins un JobDetail est TERMINE
        locations_without_terminated = []
        for (location_id, counting_id), jd_list in job_details_by_location.items():
            # Vérifier si au moins un JobDetail de cette location est TERMINE
            has_terminated = any(jd.status == 'TERMINE' for jd in jd_list)
            if not has_terminated:
                # Récupérer la référence de la location pour l'affichage
                location_ref = jd_list[0].location.location_reference if jd_list[0].location else "N/A"
                locations_without_terminated.append({
        ...
        if locations_without_terminated:
            total_locations_without_terminated = len(locations_without_terminated)
            total_job_details_non_terminated = sum(loc['count'] for loc in locations_without_terminated)
            result['skipped_non_terminated'] = 1
            self.stdout.write(
                self.style.WARNING(
                    f'  ⚠️  Assignment {assignment.reference} ignoré : '
                    f'{total_locations_without_terminated} emplacement(s) sans JobDetail TERMINE '
                    f'({total_job_details_non_terminated} JobDetail(s) non TERMINE sur {total_job_details} total) '
                    f'pour le counting {counting_order}'
                )
            )
            # Afficher les emplacements sans JobDetail TERMINE
            for loc_info in locations_without_terminated[:5]:  # Limiter à 5 pour éviter trop de logs
                self.stdout.write(
                    f'    - Emplacement {loc_info["location_ref"]} : '
                    f'{loc_info["count"]} JobDetail(s) (aucun TERMINE)'
                )
                # Afficher les statuts des JobDetail pour cet emplacement
                for jd in loc_info['job_details'][:3]:  # Limiter à 3 JobDetail par emplacement
                    self.stdout.write(
                        f'      • JobDetail {jd.reference} (statut: {jd.status})'
                    )
                if loc_info['count'] > 3:
                    self.stdout.write(f'      ... et {loc_info["count"] - 3} autre(s) JobDetail')
            if total_locations_without_terminated > 5:
                self.stdout.write(f'    ... et {total_locations_without_terminated - 5} autre(s) emplacement(s)')
            return result
        
        # 2. Tous les JobDetail sont TERMINE, on peut clôturer l'assignment
        if not dry_run:
        ...
        self.stdout.write(
            self.style.SUCCESS(
                ...
                f'({total_job_details} JobDetail(s) TERMINE vérifié(s))'
            )
        )
        ...
        if not dry_run:
            ...
            if sync_result.get('synchronized', False):
                ...
                self.stdout.write(
                    f'  🔄 {created_count} CountingDetail créé(s) lors de la synchronisation '
