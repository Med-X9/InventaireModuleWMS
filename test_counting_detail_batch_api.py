#!/usr/bin/env python3
"""
Script de test pour l'API CountingDetail avec support des lots.
...
class CountingDetailBatchTester:
    def __init__(self):
        self.token = None
        self.headers = {}
    ...
    def test_update_existing(self):
        """Test de mise à jour d'un enregistrement existant."""
        print("\n🔄 Test de mise à jour d'un enregistrement existant...")
        
        # Données pour mettre à jour un enregistrement existant (même counting + location + product)
        update_data = {
            "batch": True,
            "data": [
                {
                    "counting_id": 1,
                    "location_id": 1,  # Même emplacement que le premier test
                    "quantity_inventoried": 20,  # Nouvelle quantité
                    "assignment_id": 1,
                    "product_id": 1,  # Même produit
                    "dlc": "2025-01-15",  # Nouvelle DLC
                    "n_lot": "LOT123UPDATED",  # Nouveau lot
                    "numeros_serie": [
                        {"n_serie": "NS001_UPDATED"},
                        {"n_serie": "NS001_NEW"}
                    ]
                }
            ]
            ...
            if response.status_code == 201:
                ...
                if data.get('success'):
                    ...
                    if result['results']:
                        ...
                        print(f"   - Action: {item['result']['action']}")
                        cd = item['result']['counting_detail']
                        print(f"   - ID: {cd['id']}")
                        print(f"   - Nouvelle quantité: {cd['quantity_inventoried']}")
                        print(f"   - Dernière modification: {cd['updated_at']}")
                        
                        if item['result'].get('numeros_serie'):
                            print(f"   - Numéros de série mis à jour: {len(item['result']['numeros_serie'])}")
    ...
    def run_all_tests(self):
        ...
        print("🚀 Démarrage des tests de l'API CountingDetail avec support des lots")
        print("=" * 70)
        ...
        tests = [
            ...
            ("Mise à jour d'enregistrement existant", self.test_update_existing)
        ]
...
def main():
    """Fonction principale."""
    tester = CountingDetailBatchTester()
    success = tester.run_all_tests()
    
    if success:
        print("\n✅ L'API CountingDetail avec support des lots fonctionne correctement!")
        sys.exit(0)
    else:
